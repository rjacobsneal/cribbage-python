...
def greedy_throw(game, deal, crib):
    ...
    def score_split(indices):
        ...
        return (
            keep,
            throw,
            score(game, keep, None, False)[0]
            + crib * score(game, throw, None, True)[0],
        )
...
def score(game, hand, turn, crib):
    ...
    flushes = 0
    max_flush, long_suit = max(
        ((suit_count[s], s) for s in game.all_suits()), key=lambda p: p[0]
    )
    if (
        max_flush == len(hand)
        and not crib
        and (turn is None or turn.suit() != long_suit)
    ):
    ...
    return (
        pairs + fifteens + straights + flushes + nobs,
        pairs,
        fifteens,
        straights,
        flushes,
